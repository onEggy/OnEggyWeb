  ...
  return (
    <>
      ...
      <article className="relative max-w-4xl mx-auto px-6 py-8 md:py-16 space-y-10 overflow-hidden">
        ...
        <Link
          href="/blog"
          className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-primary-strong transition-colors group cursor-pointer"
        >
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-center gap-4">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-sm font-mono font-semibold uppercase tracking-wider text-primary-strong">
              <Tag className="h-3.5 w-3.5" aria-hidden="true" /> {post.category}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6 pt-2 text-sm text-muted-foreground border-b border-border pb-6">
            <span className="flex items-center gap-1.5">
              <Calendar className="h-4 w-4 text-primary" aria-hidden="true" /> {post.date}
            </span>
            <span className="flex items-center gap-1.5">
              <Clock className="h-4 w-4 text-primary" aria-hidden="true" /> {post.readTime}
            </span>
            <span className="flex items-center gap-1.5">
              <User className="h-4 w-4 text-primary" aria-hidden="true" /> OnEggy Engineering
            </span>
          </div>
        </div>
        ...
        {/* Main Banner Image */}
        {post.mainBigImage && (
          <div className="relative w-full aspect-[21/9] rounded-2xl overflow-hidden border border-border bg-muted shadow-2xl">
            <Image
        ...
        <div
          className="blog-prose prose max-w-3xl mx-auto text-foreground leading-relaxed space-y-6 text-base"
          dangerouslySetInnerHTML={{ __html: post.htmlContent }}
        />

        {/* Keywords / Tags Row */}
        {post.keywords.length > 0 && (
          <div className="border-t border-border pt-8 mt-12 space-y-3">
            <span className="text-sm font-mono font-semibold uppercase tracking-wider text-foreground flex items-center gap-1.5">
              <Tag className="h-4 w-4 text-primary" aria-hidden="true" /> Tag Ecosystem
            </span>
            <div className="flex flex-wrap gap-2">
              {post.keywords.map((tag) => (
                <span
                  key={tag}
                  className="px-2.5 py-1 rounded-full bg-primary/10 border border-primary/20 text-sm font-medium text-primary-strong hover:border-primary/40 transition-colors"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="surface-card p-6 md:p-8 rounded-2xl border border-border flex flex-col sm:flex-row items-center gap-6 mt-12 relative overflow-hidden">
          <div className="w-14 h-14 rounded-full bg-primary/10 border border-primary/20 text-primary-strong font-mono flex items-center justify-center font-bold text-xl shrink-0 shadow-lg">
            OE
          </div>
        ...
        {relatedPosts.length > 0 && (
          <section className="border-t border-border pt-10 mt-12 space-y-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {relatedPosts.map((related) => (
                <Link
                  ...
                  className="surface-card group flex flex-col gap-3 p-6 rounded-2xl border border-border hover:border-primary/40 transition-colors focus-visible:ring-2 focus-visible:ring-primary"
                >
                  <span className="inline-flex w-fit items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-sm font-mono font-semibold uppercase tracking-wider text-primary-strong">
                    <Tag className="h-3.5 w-3.5" aria-hidden="true" /> {related.category || "DevOps"}
                  </span>
                  <h3 className="text-base font-semibold text-foreground leading-snug group-hover:text-primary-strong transition-colors">
                    {related.title}
                  </h3>
                  {related.overview && (
