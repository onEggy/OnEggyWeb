...
export function ExpertiseNarrative() {
  const pillars = [
    ...
    {
      icon: <GitBranch className="h-5 w-5 text-primary/80" />,
    ...
    {
      icon: <Bell className="h-5 w-5 text-primary/60" />,
      label: "Observability",
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-24 border-t border-border/40 relative">
      <div className="absolute top-2 left-6 font-mono text-[8px] text-muted-foreground/30 select-none">
        GRID.SEC.B // PHILOSOPHY.ENGINEERING_V1.1
      </div>
          ...
          {pillars.map((pillar) => (
            ...
            >
              <div className="space-y-4">
                <div className="w-10 h-10 rounded-lg bg-background border border-border flex items-center justify-center shrink-0 shadow-sm group-hover:scale-105 transition-transform duration-300">
                  {pillar.icon}
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-primary/95 font-semibold uppercase tracking-wider block">
                    {pillar.label}
                  </span>
