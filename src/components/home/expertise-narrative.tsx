...
export function ExpertiseNarrative() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-24 border-t border-border/40 relative">
      <div className="absolute top-[20%] left-[10%] w-[350px] h-[350px] rounded-full bg-cyan-500/5 blur-[100px] pointer-events-none -z-10" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <FadeUp delay={0.1}>
            <h2 className="text-3xl sm:text-5xl font-bold tracking-tight text-foreground leading-[1.1]">
              Platform Engineering is an <span className="text-cyan-400">Architecture Decision</span>
            </h2>
          </FadeUp>
          <FadeUp delay={0.2}>
        ...
        <StaggerContainer className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-3 gap-6 items-stretch border-t lg:border-t-0 border-border/20 pt-8 lg:pt-0">
          {pillars.map((pillar) => (
            <StaggerItem
              key={pillar.title}
              className="glass-card p-6 rounded-xl border border-border/40 bg-background/20 hover:border-cyan-500/30 flex flex-col justify-between transition-all duration-300 group"
            >
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-cyan-500 font-semibold uppercase tracking-wider block">
                    {pillar.label}
                  </span>
